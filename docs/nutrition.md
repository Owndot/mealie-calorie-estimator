...
Strict validation rejects arbitrary text, extra keys, changed coverage/indexes, invalid numbers, invented brands, conflicting explicit states and lost nutrition-changing qualifiers. A malformed row cannot invalidate its valid neighbors. Section headings may return null. Structured g/kg/mg/oz/lb quantities and explicit Mealie custom-unit mass conversions override LLM masses. Explicit leading g/kg/mg quantities in unparsed text also override model masses. Servings never scale ingredients.

Validated complete responses are cached by input, model, endpoint and interpretation/data versions; serving-only changes can reuse normalization. Failed or partially invalid responses are not cached. On request/validation failure in batch mode, the existing interpreter runs locally without additional per-ingredient LLM classification. Quantity and nutrient fallbacks remain available. `LLM_NORMALIZE_RECIPE=false` explicitly selects that recovery path; disabling the LLM leaves local interpretation and database matching available.
...
Carbohydrates follow the existing available-carbohydrate convention: USDA fiber is subtracted once by the importer. Unknown profile fields remain null. If any included ingredient lacks a nutrient, that entire recipe nutrient is marked unknown and omitted from the Mealie patch, with a warning. This avoids presenting a known subtotal (including zero) as a complete total. Ingredient-level profiles retain the available values. No unknown value is synthesized by the final arithmetic.
...
## Realistic regression validation

`tests/realistic-pipeline.test.ts` runs German mixed-unit recipes, the 8-serving pasta regression, vague quantities, OFF candidate selection/outages and cold/warm request counts. LLM/OFF responses are fixtures; SQLite and the arithmetic/provider/payload code are real. The Docker smoke test also checks that an externally resolved branded nutrient profile survives container restart without another OFF request. It does not establish live-model accuracy.

For 15 distinct ingredients on a cold cache, batch mode uses one normalization request, 0–15 eligible OFF searches (up to 60 HTTP attempts at the default three retries), and 0–15 LLM nutrient estimates (up to 30 requests including one corrective retry each). A successful unchanged warm-cache rerun needs no external requests. Malformed batch output cannot fan out into classification calls; local recovery may still request missing weights (up to one call per ingredient) and nutrients. Failed/partial normalization is not cached; unchanged webhook attempts remain governed by existing input hashes.
