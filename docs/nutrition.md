...
3. Classified generic foods without a trustworthy state-specific reference go directly to validated LLM nutrition fallback; they do not search packaged OFF products. Branded/commercial products use OFF, checking brand against product name/brand metadata as well as food identity and state. Broad valid composite foods (sauces, pastes, marinades, dressings, condiments and seasoning mixes) may use a low-confidence generic-composite context after classification fails: OFF is attempted first, then validated LLM nutrient fallback without inventing a precise identity. Garbage, contradictory states and invalid quantities remain unmatched. If classification is disabled/unavailable by configuration, unresolved names retain the strict literal OFF path for backward compatibility.
